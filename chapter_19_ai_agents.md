...
- **통신 지연**: Gazebo 안에서는 토픽 전달이 즉시지만, 실제로는 수~수십 ms의 지연이 있다
...
- AI가 못하는 것 (19장 앞부분 참조): ROS QoS, 하드웨어 권한, 네트워크 설정, 실시간 타이밍
...
14:00 — SLAM 코드 디버깅. ROS2 에러 → 19장 참고해서 직접 해결 (AI는 QoS를 모른다)
