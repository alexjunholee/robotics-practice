...
로봇이 낯선 환경에서 "나는 어디에 있고, 주변은 어떻게 생겼는가?"를 동시에 알아내는 문제가 SLAM이다. GPS가 안 되는 실내, 지하, 건물 내부에서 로봇이 자율적으로 움직이려면 SLAM이 필요하다. 로봇 소프트웨어 엔지니어에게 가장 자주 요구되는 기술 중 하나이므로, 이론과 실습 모두 탄탄히 잡아야 한다.
